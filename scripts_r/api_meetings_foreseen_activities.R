...
#------------------------------------------------------------------------------#
## Extract PROC_ID and PROCEDURE_ID from structured_content_en -----------------
# Extract proc_id: resource="proc/YYYY-NNNN"
votes_foreseen[, proc_id := stringr::str_extract(
  string = structured_content_en,
  pattern = '(?<=resource="proc/)([^"]+)'
)]

# Extract procedure_id: the visible label like "2025/2223(INS)"
# Match the full pattern and extract what's between the tags
votes_foreseen[, procedure_id := stringr::str_match(
  string = structured_content_en,
  pattern = '<label typeof="proc"[^>]*>([^<]+)</label>'
)[, 2]]
...
process_ids = sort(
  unique(gsub(pattern = "eli/dl/proc/", replacement = "",
...
# Grab cols also from raw foreseen activities
process_tmp = votes_foreseen[, list(
  id = unlist(inverse_was_scheduled_in),
  procedure_id)
]

# Check if all PROCESS IDs are in PROCEDURES - if not, MERGE
if ( !all(process_tmp$id %in% procedures_process_id$id) ) {
    cat("\nThe Procedure endpoint is missing some PROCESS IDs - merging to complete.\n")
  procedures_process_id[
    process_tmp,
    on = c("id", "procedure_id")
  ]
}
...
# Delete cols
procedures_doc_id[, "identifier2" := NULL]
