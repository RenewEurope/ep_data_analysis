...
renew_othermeps <- renew_mjrt[,
                              list(rcv_id, result) # subset cols
][  
  meps_rcv_mandate[
...
rcvid_since202506 = unique(pl_votes$rcv_id[
  pl_votes$activity_date >= as.Date("2025-06-01")
  & pl_votes$decision_method == "VOTE_ELECTRONIC_ROLLCALL"
])

# Check whether same vote
renew_othermeps[, `:=`(
  is_same = as.integer(result == i.result),
  is_since202506 = ifelse(rcv_id %in% rcvid_since202506, "After June 2025", "Before June 2025")
)]

# take the average for plotting
dist_avg_dt <- renew_othermeps[, list(
  same_avg = mean(is_same, na.rm = TRUE) ),
  by = list(pers_id, is_since202506)
]
```
...
When reading these figures remember that **high numbers stand for high affinity**, and **low numbers represent low affinity**.
```{r}
#| fig-width: 8
#| fig-height: 4
...
dist_avg_dt_plot |> 
  ...
  scale_fill_manual(
    values = c(
      `After June 2025` = "#67a9cf", 
      `Before June 2025` = "#ef8a62")
    ) +
...
ggsave(here::here("figures", "affinity_renew_meps_target.pdf"),
...
last_value = is_same_date |> 
  ...
  dplyr::select(date, rcv_id, is_same, cum_same,polgroup_id, pers_id) |> 
  join_polit_labs() |> 
  join_meps_names() |> 
  dplyr::mutate(
...
is_same_date |> 
  ...
  facet_wrap(~mep_name, nrow=4) + # change this 2 if printing pdf
  scale_x_discrete(
    breaks = c(as.character(min(is_same_date$rcv_id)),
               as.character(max(is_same_date$rcv_id))),
    labels = c("July 2024", month_year) ) +
  labs(x="Time", y="Cumulative Average Similarity",
       title = "Cumulative alignment between Renew's majority and other MEPs",
       subtitle = stringr::str_wrap(
         "Pale dots represent cumulative mean alignment. The thicker solid line is the trend line over the entire period considered.", 
         width = 130) ) +
...
ggsave(filename = here::here("figures", "cumulative_affinity_target.pdf"),
...
* Between the 9th and 10th mandate, **Elena KOUNTOURA** was a member/Vice-Chair of: Committee on Transport and Tourism (Vice-Chair); Committee on Women’s Rights and Gender Equality; Special Committee on Artificial Intelligence in a Digital Age
...
rcvid_tokeep = pl_votes$rcv_id[
  pl_votes$activity_date >= as.Date("2025-05-01") 
  & pl_votes$activity_date <= as.Date("2025-06-30")
  & pl_votes$decision_method == "VOTE_ELECTRONIC_ROLLCALL"
]
...
pl_votes_10[, vot_id := gsub(pattern = "eli/dl/event/", replacement = "", 
                             x = inverse_consists_of)]
rcvid_ukraine = pl_votes_10[vot_id %in% ukraine_votes][["rcv_id"]]
...
dplyr::left_join(
  ...
  join_meps_names() |> 
  left_join(
    y = pl_votes_10[, list(vot_id, rcv_id)],
    by = "rcv_id"
  ) |> 
